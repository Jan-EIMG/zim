...
openssl req -x509 -newkey rsa:2048 -keyout my_private_key1.pem -out my_cert1.pem -days 355 -nodes -addext "subjectAltName = URI:urn:freeopcua:python:server,IP:192.168.81.174" -addext "keyUsage = nonRepudiation, digitalSignature, keyEncipherment, dataEncipherment, keyCertSign" -addext "extendedKeyUsage= serverAuth, clientAuth"
openssl x509 -outform der -in my_cert1.pem -out my_cert1.der

openssl req -x509 -newkey rsa:2048 -keyout my_private_key2.pem -out my_cert2.pem -days 355 -nodes -addext "subjectAltName = URI:urn:freeopcua:python:server,IP:192.168.81.174" -addext "keyUsage = nonRepudiation, digitalSignature, keyEncipherment, dataEncipherment, keyCertSign" -addext "extendedKeyUsage= serverAuth, clientAuth"
openssl x509 -outform der -in my_cert2.pem -out my_cert2.der
